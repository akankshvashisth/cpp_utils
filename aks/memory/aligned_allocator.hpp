...
void aligned_deallocate(void* new_data) {
#ifdef _WIN32
  _aligned_free(new_data);
#else
  free(new_data);
#endif
}
...
class aligned_allocator {
 public:
  ...
  constexpr static size_t const alignment_value = Alignment;

  T* address(T& r) const { return &r; }

  T const* address(T const& s) const { return &s; }
  ...
  template <typename U>
  struct rebind {
    typedef aligned_allocator<U, alignment_value> other;
  };

  bool operator!=(aligned_allocator const& other) const {
    return !(*this == other);
  }

  void construct(T* const p, T const& t) const {
  ...
  bool operator==(aligned_allocator const& other) const { return true; }
  ...
  template <typename U>
  aligned_allocator(aligned_allocator<U, alignment_value> const&) {}
  ...
  T* allocate(std::size_t const n) const {
    ...
    void* const pv = aligned_allocate(n * sizeof(T), alignment_value);
  ...
  template <typename U>
  T* allocate(std::size_t const n, U const* /* const hint */) const {
    return allocate(n);
  }
 ...
 private:
  aligned_allocator& operator=(aligned_allocator const&) = delete;
};
